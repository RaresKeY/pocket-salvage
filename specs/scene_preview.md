...
`Presentation/HUD` is a CanvasLayer with native-resolution Controls and a shared scene Theme. The contributed coin and timer icons accompany static `000` and `--:--` placeholders. The footer credits Dale Mooney and shows camera controls. Mouse wheel zooms smoothly toward the pointer, left or middle drag pans, and F resets the view. There is no timer progression, scoring, crane movement, sorting, or rope integration.
