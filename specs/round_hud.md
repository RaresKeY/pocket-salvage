...
Code structure (2026-09-26 tidy, no behaviour change): `_ready` calls `_build_header`, `_build_footer`, `_build_modal` and `_build_version`; `present` calls `_present_counters`, `_present_audio`, `_present_developer` and `_present_modal`. Widgets come from `_box`, `_button`, `_toggle_button`/`_set_toggle` ("<name> on/off"), `_label` and `_slider_row`. `HINTS` maps control scheme to hint text; `DEBUG_TOGGLES` lists the developer checkboxes; breakpoints (`COMPACT`, `TINY_WIDTH`, `NARROW_WIDTH`, `MENU_WIDTH`, `STACKED_HEADER`) and theme colours (`YardTheme.PANEL`, `PRIMARY`, `FEEDBACK`, `FAINT`, `SCRIM`, `BUTTON_FILLS`) are named once. The header's right gap is `FullscreenControl.RESERVED_WIDTH`. `world_right_inset` was removed (it was always 0).
