...
`labs/rope/` is the standalone interactive showcase, using `labs/shared/lab_page.gd` for responsive controls, viewport fitting and bounded capture. The existing pixel lab remains the startup scene. Art filtering conventions are unchanged; antialiased procedural cable lines are distinct from pixel-texture sampling.
