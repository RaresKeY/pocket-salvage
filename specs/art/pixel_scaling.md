...
`assets/pixel_lab/calibration.png` is a reproducible diagnostic chart authored by `tools/pixel_art/make_fixture.gd`, not selected game art. It tests checkers, a diagonal, one-pixel lines, and transparency. `scenes/main.tscn` now owns the separate [scrapyard scene preview](../scene_preview.md).
