...
The standalone pixel-scaling scene links to this scene through its Sprite playground button; the scrapyard preview is the project startup. Tests verify exact enlarged data and hashes for all 110 files, gallery count, gravity/floor containment, body versus sprite scale, collision dimensions, filter toggling, viewport mouse picking/release and reset. `labs/capture sprite_playground` provides separate background hardware-renderer evidence. No performance or exported-game acceptance is claimed.
