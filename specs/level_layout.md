...
The skyline is a mix, not one repeated tile. `skyline_set(name)` loads every existing variant (`name`, `name_b`, `name_c`) through `YardArt.cut_sky`, which removes each tile's painted sky (the flat bands that fill at least 40% of a row and are not the bottom row's ground colour) from the 1x source before enlarging, so tiles side by side share the yard's own sky. `skyline_plan(widths, span, seed)` lays them out with a fixed seed (`SKYLINE_SEED`), never repeating a design twice in a row and mirroring tiles at random; a single design alternates its facing. `yard_ambience.gd` continues the sky gradient below the skyline down to the fence top (`HORIZON_GLOW`, `BLOOD_HORIZON_GLOW`) so the cut-out tiles keep their horizon glow. Blood Moon uses the `backdrop_blood_skyline_tile` set. `tests/level_test.gd` checks the plan; `tests/blood_moon_test.gd` checks which set each level uses.
