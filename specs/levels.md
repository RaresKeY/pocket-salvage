...
Reviewed: 2026-09-26, Level 6 change on top of `4cf0d3d`.
...
`scripts/level/blackout.gd` runs only while the round is running: waiting (first 20 to 30 s, then 35 to 50 s), warning (2 s: both floodlights and the beacons follow `stutter`, `crackle` plays, a message), out (5 to 7 s: lights and beacons 0, a lightning strike through `weather.strike()`, `power_down`, `cut_power`; `ambience.blackout` rises to 1 over 0.3 s), then recovery (`restore_power`, lights 1, `power_up`, "Power's back."; the dark fades over 1 s).

The round: `powered()` is false while any event reports `generator_running == false`; `head_has_power()` is true for a head that is not the electric one, otherwise it needs no power cut and `powered()`. `try_pickup` requires `head_has_power()`.

`yard_ambience.gd`: `left_lamp_level` joins `right_lamp_level` (read through `lamp_level(index)`), `beacon_level` dims the corner beacons, and `blackout` sets the alpha of a near-black layer (82% at full) at z 15, under the lightning flash and bolt.

Verification: `tests/blackout_test.gd` covers the level entry, the first wait, the warning (crackle, message, both lamps and beacons stuttering, power still on), the blackout (strike, power-down, dropped load, message, darkness, magnet dead, claw working, swapping back not restoring the magnet), recovery (sound, lights, next wait, dark fading), pausing and a Level 6 win. Progression and selection tests read the level count from the catalog. Full suite passed (`CHECKS_OK`). Rendered captures (local): `Downloads/pocket-salvage/electric-storm/`.
