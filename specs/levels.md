...
`scripts/level/storm_front.gd` runs only while the round is running: phases calm (40 to 60 s), building (15 s), storm (30 to 40 s), clearing (10 s), repeating. It sets `weather.intensity` (0.12 calm, smoothstep up and down, 1 in the storm) and `ambience.gloom` (0 to 1). Building says "Storm building..." and plays a quiet thunder; storm and clearing also say so. Each storm sends one twister 4 to 12 s in.
