...
These 75 accepted PNG derivatives replicate each original pixel as an 8×8 block with nearest-neighbor scaling. Originals remain in `../bitwright/`. Masks receive the same multiplier to preserve alignment. Adjacent `.png.json` records contain source/output hashes, dimensions, factor and engine provenance.
