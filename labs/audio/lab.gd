...
func _ready() -> void:
	...
	for cue in [&"ui_click", &"start", &"pickup", &"clank", &"claw_open", &"claw_shut", &"magnet_on", &"magnet_off", &"correct", &"wrong", &"eject", &"land", &"tick", &"finish", &"trolley_loop", &"winch_loop", &"rain_slight_loop", &"rain_loop", &"rain_violent_loop", &"music_yard"]:
		var button := Button.new()
		button.text = String(cue).capitalize()
		button.custom_minimum_size = Vector2(170, 44)
		grid.add_child(button)
		if cue in [&"trolley_loop", &"winch_loop", &"rain_slight_loop", &"rain_loop", &"rain_violent_loop", &"music_yard"]:
