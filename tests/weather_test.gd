...
func run() -> void:
	...
	loud.start_round()
	await frames(5)
	for sound in [&"wind_loop", &"rain_violent_loop", &"thunder"]: assert(load("res://assets/audio/%s.wav" % sound) is AudioStreamWAV, "%s exists" % sound)
	assert(loud.sfx.loop_level(&"rain_violent_loop") == 1.0 and loud.sfx.loops.has(&"wind_loop"), "storm runs rain and wind loops")
	loud.sfx.set_effects_enabled(false)
	assert(loud.sfx.loops[&"rain_violent_loop"].current == 0.0 and loud.sfx.loops[&"wind_loop"].current == 0.0, "SFX off silences rain and wind")
	...
	noisy.start_round()
	await frames(10)
	assert(noisy.sfx.loop_level(&"rain_violent_loop") == 1.0 and noisy.sfx.loop_level(&"wind_loop") > 0.0)
	noisy.toggle_pause()
	await frames(2)
	assert(noisy.sfx.loop_level(&"wind_loop") == 0.0 and noisy.sfx.loop_level(&"rain_violent_loop") == 0.0, "weather sounds stop while paused")
	noisy.toggle_pause()
	await frames(2)
	assert(noisy.sfx.loop_level(&"rain_violent_loop") == 1.0, "rain comes back on resume")
	noisy.forced_weather = &"clear"
	noisy.restart_round()
	await frames(2)
	assert(noisy.sfx.loop_level(&"rain_violent_loop") == 0.0 and noisy.sfx.loop_level(&"wind_loop") == 0.0, "a clear round after a storm is quiet")
	noisy.queue_free()
	await process_frame
