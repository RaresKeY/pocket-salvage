...
func run() -> void:
	...
	for i in 12: assert(game.hud.level_buttons[i].disabled == (i >= 6))
	game.select_level(6)
	game.select_level(-1)
	...
	game._command(&"settings_right")
	assert(game.selected_level == 3)
	game._command(&"settings_right")
	assert(game.selected_level == 4 and game.weather.profile.id == &"storm")
	game._command(&"settings_right")
	assert(game.selected_level == 5 and game.weather.profile.id == &"electric_storm")
	game._command(&"settings_right")
	assert(game.selected_level == 5)
	for step in 3: game._command(&"settings_left")
