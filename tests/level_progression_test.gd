...
func run() -> void:
	...
	for level in Levels.LEVELS.size():
		for variant in 2:
	...
	for level in Levels.LEVELS.size():
		...
		assert(game.selected_level == mini(level + 1, Levels.LEVELS.size() - 1))
	game.start_round()
	...
	assert(game.round_state.state == &"running" and game.selected_level == Levels.LEVELS.size() - 1)
	game.round_state.tick(241)
	game.hud.levels_button.pressed.emit()
	assert(game.round_state.state == &"ready" and game.selected_level == Levels.LEVELS.size() - 1)
