...
func _run() -> void:
	...
	await process_frame
	var weathered := Round.new()
	root.add_child(weathered)
	weathered.configure(10, 2, 1.5)
	weathered.start()
	weathered.accept_delivery(1, &"steel", &"steel")
	weathered.accept_delivery(2, &"steel", &"steel")
	var before_bonus: int = 200 + weathered.time_bonus
	check(weathered.state == &"finished" and weathered.weather_bonus == roundi(before_bonus * 0.5), "multiplier adds a weather bonus on top of points and time bonus")
	check(weathered.score == before_bonus + weathered.weather_bonus, "weather bonus lands in the score once")
	weathered.start()
	weathered.accept_delivery(1, &"steel", &"rubber")
	weathered.finish(&"timeout")
	check(weathered.score == -25 and weathered.weather_bonus == 0, "no multiplier on a negative score")
	weathered.configure(10, 2)
	check(weathered.multiplier == 1.0 and weathered.weather_bonus == 0, "configure defaults to no weather")
	weathered.queue_free()
