...
# Static terrain visibility is prepared with the chapter, not on first throw.
const CLEARANCE := 2.5
...
func build(strips: Array, spikes: Array) -> void:
	solids.clear()
	corners.clear()
	links.clear()
	bends.clear()
	for strip in strips:
		var polygon := PackedVector2Array(strip)
		polygon.append(Vector2(strip[-1].x,1200))
		polygon.append(Vector2(strip[0].x,1200))
		solids.append(polygon)
	for spike in spikes: solids.append(PackedVector2Array(spike))
